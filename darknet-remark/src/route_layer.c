...
//https://www.jianshu.com/p/b68cb90811eb
void forward_route_layer(const route_layer l, network net)
{
    int i, j;
    int offset = 0; //偏移量offset
    for(i = 0; i < l.n; ++i){ //  //遍历route_layer的输入层 
        int index = l.input_layers[i];
        float *input = net.layers[index].output; //input指向所遍历层的output层
        int input_size = l.input_sizes[i];  //设置input_size为所遍历层的输入大小
        for(j = 0; j < l.batch; ++j){
            copy_cpu(input_size, input + j*input_size, 1, l.output + offset + j*l.outputs, 1);
            //将 X copy to Y,数量是N个,间隔是incx和incy
            //将所遍历层的网络输出一个个copy至大的输出层中
        }
        offset += input_size; ////偏移量为所遍历层的大小
    }
}
//http://www.360doc.com/content/17/1017/09/42392246_695618898.shtml
