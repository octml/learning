...
void forward_local_layer(const local_layer l, network net)
{
    int out_h = local_out_height(l);
    int out_w = local_out_width(l);
	//这两个函数同样是计算卷积后的图像的高度和宽度，和我们之前的卷积层计算公式对比 (l.h + 2*l.pad - l.size) / l.stide +1 
    //和卷积层不一样的是这里没有考虑pad。
    int i, j;
    int locations = out_h * out_w;
    ...
    for(i = 0; i < l.batch; ++i){
        ...
        for(j = 0; j < locations; ++j){
            float *a = l.weights + j*l.size*l.size*l.c*l.n; //权重不共享
            float *b = net.workspace + j; //我们这里的参数b是变化的，这个恰恰符合了local layer的原理
            float *c = output + j;
			...
			//我们可以对比之前的卷积层，对比后发现唯一一个区别就是参数b，我们这里的参数b是变化的，这个恰恰符合了local layer的原理。
			//
			//gemm 每次计算一列im[] ,然后修改权值,local layer就是一种权重不共享的卷积层
        }
    }
