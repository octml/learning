...
void predict_classifier(char *datacfg, char *cfgfile, char *weightfile, char *filename, int top)
{
    ...
    int i = 0;
    char **names = get_labels(name_list); //ready for show in HT
    /*names[]={ "kit fox",  //imagenet.shortnames.list
                "English setter",
                "Siberian husky",
                "Australian terrier",
                "English springer",
    ...
    */
    clock_t time;
    int *indexes = calloc(top, sizeof(int)); //save the index of result
    char buff[256];
    char *input = buff; // point initlization
    int size = net.w; //from cfg [net] section
    while(1){
        if(filename){
            strncpy(input, filename, 256);
            //strncpy(dest,src,n);    
        ...
        float *X = r.data;
        time=clock();
        float *predictions = network_predict(net, X); //return (float *)net.output
        if(net.hierarchy) hierarchy_predictions(predictions, net.outputs, net.hierarchy, 1, 1);
        //hierarchy_predictions(float *predictions, int n, tree *hier, int only_leaves, int stride)
        top_k(predictions, net.outputs, top, indexes);
        //predictins[net.outputs]是概率,从中找出前top个最大値,序号存在indexes[]
        printf("%s: Predicted in %f seconds.\n", input, sec(clock()-time));
        for(i = 0; i < top; ++i){
            int index = indexes[i];
